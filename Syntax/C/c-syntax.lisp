...
(defmethod lex ((syntax c-syntax) (state lexer-toplevel-state) scan)
  (macrolet ((fo () `(drei-buffer:forward-object scan)))
	...
	(#\# (let ((bolp (drei-buffer:beginning-of-line-p scan)))
	       (fo) 
	       (if bolp
    ...
    (cond ((drei-buffer:end-of-line-p scan)
	  ...
	  ((eql (drei-buffer:object-after scan) #\\)
	   ...
	   (if (and (not (drei-buffer:end-of-buffer-p scan))
		    (drei-buffer:end-of-line-p scan))
	  ...
	  ((c-constituentp (drei-buffer:object-after scan))
	   (loop until (or (drei-buffer:end-of-buffer-p scan)
			   (not (c-constituentp (drei-buffer:object-after scan))))
		 do (fo))
  ...
  (let ((dots-seen 0))
	...
	(incf dots-seen))
      (loop until (drei-buffer:end-of-buffer-p scan)
	    for next = (drei-buffer:object-after scan)
...
(defun lex-token (syntax scan)
  ...
  (labels ((fo () (drei-buffer:forward-object scan)))
    (cond ((alpha-or-underscore-p (drei-buffer:object-after scan))
	   (let ((token (make-array 32 :element-type 'character
				    :adjustable t :fill-pointer 0)))
	     (loop until (or (drei-buffer:end-of-buffer-p scan)
			     (not (or (alphanumericp (drei-buffer:object-after scan))
				      (eql (drei-buffer:object-after scan) #\_))))
...
(defmethod drei-motion:backward-one-expression (mark (syntax c-syntax))
...
(defmethod drei-motion:backward-one-list ((mark mark) (syntax c-syntax))
  (let ((tlv (top-level-vector syntax)))
    (multiple-value-bind (form count)
...
(defmethod drei-motion:backward-one-down ((mark mark) (syntax c-syntax))
  (let ((tlv (top-level-vector syntax)))
    (multiple-value-bind (form count)
	(top-level-form-before-in-vector tlv (drei-buffer:offset mark))
      (when form
...
(defmethod drei-motion:backward-one-up (mark (syntax c-syntax))
  (let ((tlv (top-level-vector syntax)))
    (multiple-value-bind (form count)
	(top-level-form-before-in-vector tlv (drei-buffer:offset mark))
      (when form
...
(defmethod drei-motion:forward-one-down ((mark mark) (syntax c-syntax))
  (let ((tlv (top-level-vector syntax)))
    (multiple-value-bind (form count)
	(top-level-form-after-in-vector tlv (drei-buffer:offset mark))
      (when form
...
(defmethod drei-motion:forward-one-up (mark (syntax c-syntax))
  (let ((tlv (top-level-vector syntax)))
    (multiple-value-bind (form count)
	(top-level-form-after-in-vector tlv (drei-buffer:offset mark))
      (when form
...
(defun real-column-number (mark tab-width)
  (let ((mark2 (drei-buffer:clone-mark mark)))
    (drei-buffer:beginning-of-line mark2)
    (loop with column = 0
	  until (drei-buffer:mark= mark mark2)
	  do (if (eql (drei-buffer:object-after mark2) #\Tab)
		 (loop do (incf column)
