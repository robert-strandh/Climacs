...
(defun eat-escape (scan)
  ...
  (macrolet ((fo () `(drei-buffer:forward-object scan)))
    (case (drei-buffer:object-after scan)
      ((#\b #\t #\n #\f #\r #\" #\' #\\)
       (fo) t)
      (#\u
       (loop until (drei-buffer:end-of-buffer-p scan)
	     while (eql (drei-buffer:object-after scan) #\u)
	     do (fo))
       (loop until (drei-buffer:end-of-buffer-p scan)
	     for char = (drei-buffer:object-after scan)
	     with count = 0
	     while (and (characterp char)
			(digit-char-p char 16))
	     do (fo) (incf count)
	     finally (return (or (and (drei-buffer:end-of-buffer-p scan)
				      (< count 4))
				 (= count 4)))))
      ((#\0 #\1 #\2 #\3 #\4 #\5 #\6 #\7)
       (loop repeat 3
...
(defmethod lex ((syntax java-syntax) (state lexer-toplevel-state) scan)
  (macrolet ((fo () `(drei-buffer:forward-object scan)))
	...
	(#\' (fo)
	     (cond ((drei-buffer:end-of-buffer-p scan)
		    (make-instance 'incomplete-character-literal-lexeme))
		   (t (cond ((eql (drei-buffer:object-after scan) #\\)
			     (fo)
			     (if (not (drei-buffer:end-of-buffer-p scan))
	...
	(#\. (fo) (if (drei-buffer:end-of-buffer-p scan)
		      (make-instance 'dot-lexeme)
		      (cond ((and (characterp (drei-buffer:object-after scan))
				  (digit-char-p (drei-buffer:object-after scan)))
	...
	(#\* (fo) (if (drei-buffer:end-of-buffer-p scan)
		      ...
		      (cond ((eql (drei-buffer:object-after scan) #\=)
			     (fo)
    ...
    (let ((object (drei-buffer:object-after scan)))
      (cond ((eql object #\") (fo) (make-instance 'string-end-lexeme))
	    ...
	    ((java-letter-or-digit-p object)
	     (loop until (or (drei-buffer:end-of-buffer-p scan)
			     (not (java-letter-or-digit-p (drei-buffer:object-after scan))))
		   do (fo))
...
(defmethod lex ((syntax java-syntax) (state lexer-long-comment-state) scan)
  (flet ((fo () (drei-buffer:forward-object scan)))
    (let ((object (drei-buffer:object-after scan)))
      (cond ((eql object #\*)
	     (fo)
	     (cond ((or (drei-buffer:end-of-buffer-p scan)
...
(defmethod lex ((syntax java-syntax) (state lexer-line-comment-state) scan)
  (macrolet ((fo () `(drei-buffer:forward-object scan)))
    (cond ((drei-buffer:end-of-line-p scan)
	   (make-instance 'comment-end-lexeme))
	  ((java-letter-or-digit-p (drei-buffer:object-after scan))
	   (loop until (or (drei-buffer:end-of-buffer-p scan)
			   (not (java-letter-or-digit-p (drei-buffer:object-after scan))))
		 do (fo))
	   (make-instance 'word-lexeme))
...
(defun lex-number (scan)
  (let (hex oct dot exp float-suffix)
    (labels ((fo () (drei-buffer:forward-object scan))
	     (eat-digits (&optional (radix 10))
	       (loop until (drei-buffer:end-of-buffer-p scan)
		     while (and (characterp (drei-buffer:object-after scan))
				(digit-char-p (drei-buffer:object-after scan) radix))
		     do (fo))))
      (when (eql (drei-buffer:object-after scan) #\0)
	(fo)
	(cond ((drei-buffer:end-of-buffer-p scan)
	       (return-from lex-number 'decimal-integer-literal-lexeme))
	      ((equalp (drei-buffer:object-after scan) #\X)
	       (fo)
	       (setf hex t))
	      ((eql (drei-buffer:object-after scan) #\.)
	       (fo)
	       (setf dot t))
	      ((and (characterp (drei-buffer:object-after scan))
		    (digit-char-p (drei-buffer:object-after scan) 8))
	       (setf oct t))
	      ((equalp (drei-buffer:object-after scan) #\L)
	       (fo)
	       (return-from lex-number 'decimal-integer-literal-lexeme))))
	...
	(return-from lex-number
      ...
      (cond ((equalp (drei-buffer:object-after scan) #\L)
	     (fo)
	     (return-from lex-number
	       (cond (hex 'hex-integer-literal-lexeme)
		(oct 'octal-integer-literal-lexeme)
	...
	(return-from lex-number
      ...
      (when (equalp (drei-buffer:object-after scan) (if hex #\P #\E))
	(setf exp t)
	(fo)
	(when (drei-buffer:end-of-buffer-p scan)
	  (return-from lex-number 'bad-number-literal-lexeme))
	(if (member (drei-buffer:object-after scan) '(#\+ #\-))
	    (fo))
	(when (drei-buffer:end-of-buffer-p scan)
	  (return-from lex-number 'bad-number-literal-lexeme))
	(eat-digits))
      (unless (drei-buffer:end-of-buffer-p scan)
	  (when (member (drei-buffer:object-after scan) '(#\f #\F #\d #\D))
	    (setf float-suffix t)
	    (fo)))
      (return-from lex-number
	(if (or dot exp float-suffix)
...
(defmethod lex ((syntax java-syntax) (state lexer-error-state) scan)
  (macrolet ((fo () `(drei-buffer:forward-object scan)))
    (loop until (drei-buffer:end-of-line-p scan)
	  do (fo))
    (make-instance 'error-lexeme)))
...
;;; The action on drei-buffer:end-of-buffer is to reduce to the error symbol.
...
(defmethod drei-motion:backward-one-expression ((mark mark) (syntax java-syntax))
  (let ((tlv (top-level-vector syntax)))
	...
	(top-level-form-before-in-vector tlv (drei-buffer:offset mark) t)
      (when form
...
(defmethod drei-motion:forward-one-expression ((mark mark) (syntax java-syntax))
  (let ((tlv (top-level-vector syntax)))
    (multiple-value-bind (form count)
	(top-level-form-after-in-vector tlv (drei-buffer:offset mark) t)
      (when form
...
(defmethod drei-motion:forward-one-list (mark (syntax java-syntax))
  (let ((tlv (top-level-vector syntax)))
    (multiple-value-bind (form count)
	(top-level-form-after-in-vector tlv (drei-buffer:offset mark))
      (when form
	(loop for index from count below (length tlv)
	      for match = (aref tlv index)
	      with delims = ()
...
(defmethod drei-motion:backward-one-list (mark (syntax java-syntax))
  (let ((tlv (top-level-vector syntax)))
    (multiple-value-bind (form count)
	(top-level-form-before-in-vector tlv (drei-buffer:offset mark))
      (when form
	(loop for index from count downto 0
	      for match = (aref tlv index)
	      with delims = ()
...
(defmethod syntax-line-indentation (mark tab-width (syntax java-syntax))
  ...
  (let ((this-indentation (line-indentation mark tab-width)))
    ...
    (loop until (drei-buffer:beginning-of-buffer-p mark)
